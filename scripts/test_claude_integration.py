...
from src.llm.claude_config import get_default_model, is_claude_code_available
...
def test_structured_output():
    ...
    template = ChatPromptTemplate.from_messages([("system", "You are a portfolio manager. Make trading decisions based on analyst signals."), ("human", "Signals:\n{signals}\n\nAllowed actions:\n{allowed}")])

    prompt = template.invoke({"signals": json.dumps(signals, indent=2), "allowed": json.dumps(allowed, indent=2)})
