...
def create_ascii_bar(value: float, max_value: float = 100, width: int = 20, 
...
def create_portfolio_pie_chart(allocations: Dict[str, float], width: int = 40) -> str:
    ...
    for i, (ticker, value) in enumerate(sorted(allocations.items(), 
...
def print_analysis_summary(result: dict, timing_info: Optional[dict] = None) -> None:
    ...
    for ticker in decisions.keys():
        ...
        for agent_name, signals in analyst_signals.items():
            ...
            # Color-code signal
            signal_style = {
                "BULLISH": "[bold green]▲ BULL[/bold green]",
                "BEARISH": "[bold red]▼ BEAR[/bold red]",
                "NEUTRAL": "[bold yellow]◆ NEUT[/bold yellow]"
    ...
    for ticker, decision in decisions.items():
        ...
        action_display = {
            ...
            "COVER": "[bold green]📈 COVER[/bold green]"
        }.get(action, action)
    ...
    console.print()
    console.print(Panel.fit(
        "[bold]📈 PORTFOLIO ALLOCATION[/bold]",
    ...
    console.print(
        "[dim]AI Hedge Fund - Analysis powered by Claude Code CLI[/dim]",
        justify="center"
    )
    console.print(
        "[dim]⚠️  This is for educational purposes only. Not financial advice.[/dim]",
        justify="center"
    )
...
def print_agent_vote_summary(analyst_signals: dict, ticker: str) -> None:
    ...
    bar = f"[green]{'█' * bull_width}[/green]" + \
          f"[yellow]{'█' * neut_width}[/yellow]" + \
          f"[red]{'█' * bear_width}[/red]"
    console.print(f"  {bar}")
...
def print_backtesting_comparison(predictions: dict, actual_returns: dict) -> None:
    ...
    console.print()
    console.print(Panel.fit(
        "[bold]📊 BACKTESTING COMPARISON[/bold]",
        border_style="cyan"
    ))
