...
def test_long_only_strategy_buys_and_sells():
    ...
    portfolio_summary = compute_portfolio_summary(portfolio=engine._portfolio, total_value=final_portfolio_value, initial_value=initial_capital, performance_metrics=performance_metrics)
...
def test_long_only_strategy_full_liquidation_cycle():
    ...
    assert portfolio_summary["total_position_value"] == expected_total_position_value, f"Total position value should be 0 after liquidation, got {portfolio_summary['total_position_value']}"

    # Final portfolio value should equal cash balance (since no positions)
    assert abs(final_portfolio_value - final_cash) < 0.01, f"Portfolio value should equal cash after liquidation: value={final_portfolio_value}, cash={final_cash}"
...
def test_long_only_strategy_portfolio_rebalancing():
    ...
    assert portfolio_summary["total_position_value"] >= expected_min_position_value, f"Total position value should be at least {expected_min_position_value}, got {portfolio_summary['total_position_value']}"
...
def test_long_only_strategy_multiple_entry_exit_cycles():
    ...
    assert portfolio_summary["total_position_value"] == 0.0, f"Total position value should be 0 after final liquidation, got {portfolio_summary['total_position_value']}"
    assert abs(final_portfolio_value - final_cash) < 0.01, f"Portfolio value should equal cash after final liquidation: value={final_portfolio_value}, cash={final_cash}"
